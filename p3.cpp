...
long CacheFriendlySieve(long n){
    ...
    bool* composite = new bool[n + 1] ;
    memset(composite, 0, n) ;
    long *factor = new long[m] , *striker = new long[m] ;
    long n_factor = 0;
    t = omp_get_wtime() ;
    for (long i = 2; i <= m; ++i)
    if (! composite[i] ){
...
// Strike walks window of size sqrt(n) here.
    ...
    t = omp_get_wtime() - t;
    delete[] striker; delete[] factor; delete[] composite;
    return count;
}
...
#pragma omp parallel
    {
    bool* composite = new bool[m + 1] ; long* striker = new
    long[m] ;
    #pragma omp single
    {
...
// Must compute striker from scratch.
    ...
    delete[] striker; delete[] composite;
    }
    t = omp_get_wtime() - t;
    delete[] factor;
    return count;
